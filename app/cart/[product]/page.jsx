...
export default async function CartPage({ params, searchParams }) {
  ...
  try {
    ...
    const [currentUser, existingProduct] = await Promise.all([
      User.findOne({ where: { sub: id }, transaction }),
      Item.findOne({
        where: { name: product },
        include: [
    ...
    if (currentOrder) {
      const result = await renderOrderView(currentOrder, existingProduct, currentUser, product, invite, transaction);
      await transaction.commit();
      return result;
    }

    if (invite) {
      const result = await handleInviteProcess(invite, existingProduct, currentUser, product, transaction);
      await transaction.commit();
      return result;
    }
...
async function renderOrderView(currentOrder, existingProduct, currentUser, product, invite, transaction) {
  const t = await getTranslations();
...
async function handleInviteProcess(invite, existingProduct, currentUser, product, transaction) {
  ...
  const allRelatedOrders = await fetchRelatedOrders(existingProduct.id, existingInvite.id, transaction);
...
async function getOrCreateOrder(inviterId, existingProduct, inviteId, transaction) {
  ...
  let allRelatedOrders = await fetchRelatedOrders(existingProduct.id, inviteId, transaction);
...
async function fetchRelatedOrders(itemId, rootInviteId, transaction) {
...
async function updateRelatedOrders(orders, totalBuyers, discountAmount, originalPrice, transaction) {
